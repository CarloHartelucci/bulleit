...
module OauthHelper
	def redirect_uri
	    redirect_uri = "http://" << request.host_with_port << "/authorize"
	end

	FB_ACCESS_SCOPE = "user_location,user_about_me"
	def oauth_request(redirect_uri, session)
	...
	def get_profile(redirect_uri, session, params)
		code = params[:code]
    	state = params[:state]
